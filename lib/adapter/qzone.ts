...
export async function qzoneVisitorCount(account: any): Promise<string> {
	try {
		...
		const result = JSON.parse(jsondata).data.todaycount;
		return `今日浏览量为：${result}`;
	} catch (err: any) {
...
export async function qzoneBlock(account: any, act_uin: string, action: '1' | '2' = '1'): Promise<string> {
	try {
		...
		if (match && match[1]) {
			...
			return `投递空间完成：${callback.message} ${callback.data.name}`;
		} else {
			return '操作失败：未找到返回信息';
		}
	} catch (err: any) {
...
export async function qzonePublish(account: any, pics: any[], con: string, ugc_right: string = '128'): Promise<number | { error: any }> {
	try {
        ...
        console.log('qzonePublish richval:', richval);
        console.log('qzonePublish pic_bo:', pic_bo);
